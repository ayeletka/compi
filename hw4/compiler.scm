...
(define code-gen-lambda-var-body
  (lambda (sexpr envLevel paramsLevel)
    ...
    (string-append
      ...
      ; R10 holds old fp
      "POP(R10);" nl 
      ; R11 holds return addr
      "POP(R11);" nl 
      ; R12 holds env
      "POP(R12);" nl 
      ; R13 holds num of args
      "POP(R13);" nl 
...
(define code-gen-lambda-opt-body
  (lambda (sexpr envLevel paramsLevel)
    (let* (
      (numberOfParams (length (cadr sexpr)))
        (body           (cadddr sexpr))
        (copyLabel     (string-append "optLambdaCopyLabel" (labelNumberInString)))
        (endCopyLabel     (string-append "optLambdaEndCopyLabel" (labelNumberInString)))
        (pushLabel     (string-append "optLambdaPushLabel" (labelNumberInString)))
        (endPushLabel     (string-append "optLambdaEndPushLabel" (labelNumberInString)))
        )
    (string-append
      "/* lambda opt body */" nl
      ; R10 holds old fp
      ...
      ; R12 holds the env
      "POP(R12);" nl 
      ; R13 holds the numb of args
      "POP(R13);" nl 

      (malloc numberOfParams)

      ;R14 hold the space for args
      "MOV(R14, R0);" nl 
      ;R15 holds the idx
      "MOV(R15, IMM(0));" nl 
      ...
      ;R1 holds variable
      "POP(R1);" nl
      "MOV(INDD(R14,R15), R1);" nl
      "ADD(R15, IMM(1));" nl
      "JUMP(" copyLabel ");" nl
      endCopyLabel ":" nl

      "MOV(R1, R13);" nl
      ; R1 holds the num of opt args
      "SUB(R1, IMM(" (number->string numberOfParams) "));" nl 
      ...
      "DROP(IMM(1));" nl
      "POP(R1);" nl
      ;remove all opt args from list
      "DROP(R1);" nl 
      ; R2 holds the num of opt args
      "MOV(R2, R0);" nl 

      ; push all args to stack 
      "PUSH(R2);" nl
      ;R15 holds the idx
      "MOV(R15, IMM(" (number->string (- numberOfParams 1)) "));" nl 

      pushLabel ":" nl
      "CMP(R15, IMM(-1));" nl
      "JUMP_EQ(" endPushLabel ");" nl
      "PUSH(INDD(R14,R15));" nl
      "SUB(R15, IMM(1));" nl
      "JUMP(" pushLabel ");" nl
      endPushLabel ":" nl
      ...
      (code-gen  body (+ 1 envLevel)  (+ 1 numberOfParams))      
))))
...
;(compile-scheme-file "AllTests/NotPassed/a10.scm" "foo.c")

;(compile-scheme-file "AllTests/NotPassed/torture-test-for-compiler-09.scm" "foo.c")
